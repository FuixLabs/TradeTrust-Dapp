...
// * Constants libraries
import { SERVICE, IdentityProofType, VALID_DOCUMENT_NAME_TYPE, _DOCUMENT_TYPE } from '../constants/type';
...
import { _wrapDocument, _createDocument, generateDidOfWrappedDocument, unsalt } from './data';
 ...
 * @return {Promise}
 */
export const createWrappedDocument = async (document, service, walletAddress, did) => {
  ...
  try {
    ...
    const response = await checkExistsDidoWrappedDoc(CLIENT_PATH.CHECK_DID_OF_WRAPPED_DOCUMENT, {
      companyName,
      fileName: fileName,
    });
...
export const createDocuments = async (documents, usedAddress, update, updateDocument) => {
  try {
    for (let index = 0; index < documents.length; index++) {
      ...
      try {
        ...
        await sendWrappedDocument(CLIENT_PATH.SEND_WRAPPED_DOCUMENT, requestBody); // **
      } catch (e) {
        return;
      }
    }
...
export const pullNFTs = async (policyId) => {
  if (!policyId) return VERIFIER_ERROR_CODE.MISSING_PARAMETERS;
  try {
    const data = {
      policyId: policyId,
    };
    const res = await _pullNFTs(CLIENT_PATH.PULL_NFTS, data);
    if (res.data.error_code) return res.data;
 ...
 * @return {Promise}
 */
export const signObject = async (currentWallet, publicKey, data, fuixlabsWalletors) => {
  if (fuixlabsWalletors)
    return await fuixlabsWalletors(publicKey, Buffer.from(JSON.stringify(data), 'utf8').toString('hex'));
  return await currentWallet.signData(publicKey, Buffer.from(JSON.stringify(data), 'utf8').toString('hex'));
};
...
export const createCredential = async (
  { currentWallet, dids, didoWrappedDocument, metadata, action },
  currentUserPublicKey
) => {
  if (!dids || !didoWrappedDocument || !action || !currentWallet) return CREDENTIAL_ERROR.INVALID_PARAMETER;
  let transferObject = {};
  // eslint-disable-next-line array-callback-return
  action.fields.map((prop) => {
  ...
  try {
    const signedData = await signObject(currentWallet, currentUserPublicKey, payload);
    // * Convert payload object to hex string
    const hexStringPayload = Buffer.from(JSON.stringify(payload), 'utf8').toString('hex');
...
export const changeHoldership = async (currentUserPublicKey, args) => {
  const { didoWrappedDocument, keys, action, config, confirmNominate } = args;
  if (!didoWrappedDocument || !currentUserPublicKey || !keys || !action) return VERIFIER_ERROR_CODE.INVALID_PARAMETER;
  ...
  try {
    ...
    if (!isSurrender) {
      let requestResult;
      let transferPerson = 'ownerKey';
      if (!confirmNominate) {
        requestResult = await requestPublicKey(CLIENT_PATH.GET_PUBLIC_KEY, {
          address: keys[action.updatedFieds[0].name],
          user: action.updatedFieds[0].name,
          confirmNominate: confirmNominate,
        });
        transferPerson = requestResult.data.user;
      }
      dids = {
        ...dids,
        [transferPerson]: generateDid(COMPANY_NAME, requestResult?.data ? requestResult?.data?.publicKey : null),
      };
      publicKey = confirmNominate
        ? keys[action.updatedFieds[0].name]
        : requestResult?.data
    ...
    const props = {
      ...
      dids,
      metadata: {
        currentOwner: currentUserPublicKey,
      },
    };
    // * Create transferable credential depended on action type
    const result = await createCredential(props, currentUserPublicKey);
    if (action.code === 20) if (result.error_code) throw CREDENTIAL_ERROR.CANNOT_CREATE_CREDENTIAL; // * In this case, return the error code object
    ...
    const bodyData = {
      ...
      credential:
        action.code === 20
          ? { ...credential, timestamp: Date.now(), status: 'pending' }
          : { ...credential, timestamp: Date.now() },
      payload: payload,
      signature: signature,
      config: config,
    };
    const createResponse = await requestCreateCredential(CLIENT_PATH.CREATE_CREDENTIAL, bodyData);
    if (createResponse.error_code) throw createResponse;
    return {
