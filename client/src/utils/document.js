...
import { _wrapDocument, _createDocument, generateDidOfWrappedDocument, unsalt } from '../utils/data';
 ...
 * @return {Promise}
 */
export const createDocument = async (document, service, walletAddress, did) => {
...
export const createDocuments = async (documents, usedAddress, update, updateDocument) => {
  try {
    for (let index = 0; index < documents.length; index++) {
      ...
      try {
        let res = await createDocument(createdDocument, SAMPLE_SERVICE, usedAddress, did);
        if (res.error_code) {
          throw res;
        }
 ...
 * @return {Promise}
 */
export const signObject = async (publicKey, data) => {
  return await window.cardano.signData(publicKey, Buffer.from(JSON.stringify(data), 'utf8').toString('hex'));
};
 ...
 */
export const createCredential = async ({ dids, didoWrappedDocument, metadata, action }, currentUserPublicKey) => {
  if (!dids || !didoWrappedDocument || !action) return CREDENTIAL_ERROR.INVALID_PARAMETER;
...
export const changeHoldership = async (currentUserPublicKey, args) => {
  ...
  const isSurrender = ACTIONS_IDENTITY.find((_action) => _action === action).surrender;
  try {
    let dids = {
    ...
    if (!isSurrender) {
      ...
      dids = {
        ...dids,
        [requestResult.data.user]: generateDid(COMPANY_NAME, requestResult?.data?.publicKey),
      };
    ...
    if (result.error_code) throw CREDENTIAL_ERROR.CANNOT_CREATE_CREDENTIAL; // * In this case, return the error code object
 ...
 *
 */
export const pullCredential = async (hash) => {
  try {
    const credentialResult = await requestPullCredential(CLIENT_PATH.PULL_CREDENTIAL, {
      hash: hash,
    });
